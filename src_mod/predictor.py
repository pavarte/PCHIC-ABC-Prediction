...
def make_pred_table(chromosome, enh, genes, args):
    ...
    print('enh_midpoint',enh[['start','end','enh_midpoint']].head())
    ...
    print(enh_pr.head())
    print('enh pyranges head midpoint',enh_pr[['start','end','enh_midpoint']].head())
    print(genes_pr.head())
...
def add_hic_to_enh_gene_table(enh, genes, pred, hic_file, hic_norm_file, hic_is_vc, chromosome, args):
    ...
    if args.hic_type == "bedpe":
        ...
        print("Dimensions of (old overlap) enh_hic2: ", enh_hic2.shape)
        genes_hic1 = df_to_pyranges(genes, start_col = 'TargetGeneTSS', end_col = 'TargetGeneTSS', start_slop=1, end_slop = 1).join(hic1).df
        print("Dimensions of (new overlap) genes_hic1: ", genes_hic1.shape)
        ovl21 = enh_hic2[['enh_idx','hic_idx','hic_contact']].merge(genes_hic1[['gene_idx', 'hic_idx']], on = ['hic_idx'])
        print("Dimensions of ovl21: ", ovl21.shape)
...
def add_hic_pseudocount(pred, args):
    ...
    pred['hic_contact_pl_scaled_adj'] = pred['hic_contact_pl_scaled']
